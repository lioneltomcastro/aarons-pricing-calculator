...
def attendance_page():
    ...
    if register_type == "First time registration":
        ...
        st.info("Your BSB and bank account are saved only once. This first registration will be saved automatically as Check In.")
 
        # Keep this in one column so it displays correctly on mobile:
        # 1. First Name
        # 2. Last Name
        # 3. BSB
        # 4. Account Number
        ...
        if st.button("✅ Register Worker and Check In", use_container_width=True):
            ...
            if not first_name.strip() or not last_name.strip() or not bsb.strip() or not account.strip():
            ...
            if full_name not in worker_names:
                ...
                # Default rate is 35 and Sunday Rate starts at 0.
