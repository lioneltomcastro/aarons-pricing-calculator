...
if st.button("💾 Save Costing to Google Sheets"):
    ...
    float(row["Quantity Used"]),
    float(row["Your Rate"]),
    float(row["Total"]),
    float(row["Worker Days"]),
    float(equipment_cost),
    float(disposal_cost),
    float(truck_cost),
    float(other_cost),
    float(selected_multiplier_total),
    float(difficulty_allowance),
    float(labour_cost),
    float(supervisor_cost),
    float(direct_cost),
    float(recommended_price),
    float(recommended_gst),
    float(recommended_inc_gst),
    str(row["Notes"])
])
            ws.append_rows(
                rows,
