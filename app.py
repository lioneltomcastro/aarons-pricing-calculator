...
def load_rates(url):
    ...
    required = ["item", "category", "unit", "market_min", "market_recommended", "market_high", "productivity_per_day", "notes"]
...
def connect_google_sheet():
    ...
    try:
        ...
        ws = spreadsheet.add_worksheet(title="Costing_Records", rows="5000", cols="50")
        ws.append_row([
    ...
    for item_idx in range(st.session_state.room_items[room_idx]):
        ...
        if measurement_method == "Wall Calculator (Total Length x Height)":
            ...
            if unit_lower in ["lm", "linear metre", "linear meter"]:
                ...
                measurement_note = f"Calculated wall area: {wall_area:.2f} m². Pricing quantity used: {quantity:.2f} lm."
            else:
                quantity = wall_area
                measurement_note = f"Calculated wall area used for pricing: {quantity:.2f} m²."
            ...
            measurement_note = f"Calculated area used for pricing: {quantity:.2f} m²."
            st.info(measurement_note)
        ...
        with p3:
            st.caption(f"Quantity used for pricing: {quantity:.2f} {unit}")
        ...
        if your_rate < market_min:
            st.error(f"🚨 Below market minimum: ${market_min:,.2f}/{unit}")
        elif market_min <= your_rate < market_rec:
            st.warning(f"⚠️ Below recommended: ${market_rec:,.2f}/{unit}")
        elif market_rec <= your_rate <= market_high:
            st.success("✅ Within normal market range.")
        else:
            st.info(f"💎 Premium pricing. Market high: ${market_high:,.2f}/{unit}")

        m1, m2, m3, m4 = st.columns(4)
        m1.caption(f"Market Min: ${market_min:,.2f}/{unit}")
        m2.caption(f"Recommended: ${market_rec:,.2f}/{unit}")
        m3.caption(f"Market High: ${market_high:,.2f}/{unit}")
        m4.caption(f"Productivity: {productivity:g} {unit}/worker/day")

        notes = st.text_input("Item Notes", value=str(row["notes"]) if str(row["notes"]).lower() != "nan" else "", key=f"notes_{item_key}")
...
st.info(f"Estimated labour duration: **{estimated_days:.2f} days**. Rounded allowance: **{rounded_days} day(s)**.")
...
s1.metric("Base Work Items", f"${base_total:,.2f}")
s2.metric("Labour Cost", f"${labour_cost:,.2f}")
s3.metric("Difficulty %", f"{selected_multiplier_total:.1f}%")
s4.metric("Difficulty Allowance", f"${difficulty_allowance:,.2f}")

s5, s6, s7, s8 = st.columns(4)
s5.metric("Direct Cost", f"${direct_cost:,.2f}")
s6.metric("Cost + Difficulty", f"${cost_with_difficulty:,.2f}")
s7.metric("Recommended + GST", f"${recommended_price:,.2f}")
s8.metric("Total Inc GST", f"${recommended_inc_gst:,.2f}")

st.markdown("### Suggested Prices")
q1, q2, q3 = st.columns(3)
q1.success(f"Minimum Safe Price: **${minimum_price:,.2f} + GST**")
q2.info(f"Recommended Price: **${recommended_price:,.2f} + GST**")
q3.warning(f"Premium Price: **${premium_price:,.2f} + GST**")
...
if st.button("💾 Save Costing to Google Sheets"):
    if items_df.empty:
        ...
        try:
            costing_sheet = connect_google_sheet()
            ...
            costing_sheet.append_rows(rows, value_input_option="USER_ENTERED")
