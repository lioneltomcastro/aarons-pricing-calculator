...
page = st.sidebar.radio("Navigation", ["New Costing", "Saved Costings", "Manager View"])
...
if page == "Saved Costings":
    ...
    selected_quote = st.selectbox("Select Quote", summary_df["quote_id"].unique())
...
if page == "Manager View":
    ...
    if not selected_df.empty:
        ...
        with c1:
            st.metric("Client", first_row["client_name"])
            st.metric("Project Address", first_row["project_address"])

        with c2:
            st.metric("Project Type", first_row["project_type"])
            st.metric("Working Hours", first_row["working_hours"])
        ...
        f2.metric("Recommended Price", str(first_row["recommended_price"]))
        f3.metric("Final Total", str(first_row["final_total"]))
